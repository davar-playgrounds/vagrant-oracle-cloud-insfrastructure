...
1. **Question one (1)** | 30 minutes | **The SELECT performs poorly because**: MySQL reads every one of the 32769 rows in the citycodes table in order to find the result. This can be seen in the output of EXPLAIN: type is "ALL", meaning all rows are read; and, rows is "32769". 
	
	There _is_ an index on the citycodes.code column, "idx_code"; and, _if_ MySQL were to use it, MySQL would quickly scan through the index and go directly to the leaf at which citycodes.code is equal to 37040. That would be good performance.
	
	MySQL does not use the index because of a type mismatch: the query passes an integer (37040) as the value to be found in the citycodes.code column, while the type of the citycodes.code column is VARCHAR. 
	
	**A possible solution is**: Wrap 37040 in single quotes in the query, like `EXPLAIN SELECT city FROM citycodes WHERE code = '37040'\G`. That way, 37040 would be passed as a string rather than as an integer, and MySQL would use the "idx_code" index to speed the query.
	
1. **Question two (2)** | 120 minutes | **Changes that may bring performance improvements are**:
	...
	1. Make changes that enable MySQL to use an index on the enrollment table. MySQL does not use an index on it for the given SELECT because of the expression FIND_IN_SET('F', e.enrollment_flags) in the WHERE clause of the SELECT (MySQL does not use an index when matching against a derived value e.g. the result of FIND_IN_SET). Make the following changes to make the FIND_IN_SET expression unneccessary and remove it from the SELECT so MySQL can use an index:
		...
		1. Make columns for flags, in the new enrollment_flags table: For each unique flag in the comma-separated flags in the enrollment_flags column of the rows of the enrollment table, add to the enrollments_flags table a corresponding column named after the flag. For example, if the F flag appears among the comma-separated flags, then add a column enrollment_flags.F of type BOOL NOT NULL DEFAULT FALSE.
		1. Move the flags from the enrollments table to the enrollment_flags table. For each row in the enrollments table: 
			1. Create a corresponding row in the enrollments_flag table. 
				...
				2. For each flag in the comma-separated list in the enrollment_flags column of the current row: A) set the value of the respective (named for the flag) column in the corresponding row in the enrollment_flags table to TRUE, and B) remove the flag from the comma-separated list.  If the flag is the last of the comma-separated values, then set the value of enrollment_flags of the current row to NULL. For example: if F is present in the comma-separated list, then set the value of the F column in the corresponding row of enrollment_flags to TRUE ,  and then remove F from the comma-separated list of the current row of enrollment. If F is the last flag in the comma-separated list, then set the value of enrollment_flags of the current row to NULL.
...
1. **Question three (3)** | 90 minutes | **The query performs poorly in MySQL 5.0 because**: [the MySQL 5.0 optimizer incorrectly identifies the subquery as a dependent subquery](https://bugs.mysql.com/bug.php?id=32665), so the subquery is run once per row of the containing query. The execution time thus is on the order of O(rows-of-inner*rows-of-outer) rather than O(rows-of-inner+rows-of-outer). **Some solutions are**:
	1. Replace "IN" with "=": [The optimizer misidentifies the subquery as a dependent subquery because the subquery is using "IN"](https://bugs.mysql.com/bug.php?id=32665). Replacement of "IN" with "=" solves the problem. CountryCode must be removed from the subquery in order for the query to work while "IN" has been replaced with "=", because the left and right sides of "=" must be single values. CountryCode can safely be removed from the subquery: it is redundant there because CountryCode is selected in the containing query. Here is the query with "IN" replaced by "=", and CountryCode removed from the subquery:
 	...
 	1. SELECT the desired columns from a JOIN of City and an alias of City ON City.Population = MAX(AliasOfCity.Population); rather than SELECT from City WHERE Population is IN (or equal to) the result of a subquery:
...
1. **Question four (4)** | 60 minutes | **The cause of the failure of the query is**: There are INNER JOINs expressed by the comma operator (aka "comma joins") mixed with another join type i.e. LEFT JOINs. JOIN has a higher precedence than the comma operator in MySQL 5; hence, the join expression "student, enrollment, inprog LEFT JOIN clsenr" is interpreted by MySQL 5 as (student, enrollment (inprog JOIN clsenr ON clsenr.enr_id= enrollment.enr_id)). The operands of the ON clause thus are only inprog and clsenr, i.e. do not include enrollment; so, the enrollment.enr_id column is unknown to the ON clause.  **A solution is**: Avoid the use of the comma operator. Use JOIN instead:
...
1. **Question five (5)** | 240 minutes | **Suggestions I feel are called for in order to improve performance, stability, et cetera; are**: 
...
1. **Question six (6)** | 210 minutes | **The errors are**:
	1. No DELIMITER 
